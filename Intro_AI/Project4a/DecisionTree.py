...
def getAttributeCounts(examples, attrName, attrValues, className):
    ...
    for val in attrValues:
        ...
        attributeCounts[val] = getClassCounts(pertExamples, className)
...
def remainder(examples, attrName, attrValues, className):
    ...
    for val in attrValues:
        pertExamples = getPertinentExamples(examples, attrName, val)
        attributeCounts = getClassCounts(pertExamples, className)
        counts = list(attributeCounts.values())
        total = sum(counts)
        entropy = setEntropy(counts)
        ratio = total / float(len(examples))
        rem += ratio * entropy
    return rem
...
def infoGain(examples, attrName, attrValues, className):
    ...
    """
    # YOUR CODE HERE
    counts = list(getClassCounts(examples, className).values())
    return setEntropy(counts) - remainder(examples, attrName, attrValues, className)
...
def giniGain(examples, attrName, attrValues, className):
    ...
    for key in attrCounts.keys():
        val = attrCounts[key]
        giniInd = giniIndex(val.values())
        giniG += (giniInd * float(sum(list(val.values()))))

    if giniG == 0:
        return 'sys.maxint'
    return 1 / (giniG / len(examples))
...
def makeSubtrees(remainingAttributes, examples, attributeValues, className, defaultLabel, setScoreFunc, gainFunc):
    ...
    if len(examples) == 0:
        ...
        maxGain = 0.0
        best = None
        for attrib in remainingAttributes:
            temp = gainFunc(examples, attrib, attributeValues[attrib], className)
            if temp > maxGain:
                maxGain = temp
                best = attrib
        remainingAttrList = list(remainingAttributes)
        if best in remainingAttrList:
            remainingAttrList.remove(best)
        node = Node(best)
        if best is not None:
            for key in attributeValues[best]:
                defaultLabel = getMostCommonClass(examples, className)
                node.children[key] = makeSubtrees(remainingAttrList, getPertinentExamples(examples, best, key),
                                                  attributeValues, className, defaultLabel, setScoreFunc, gainFunc)
    return node
...
def makePrunedTree(examples, attrValues, className, setScoreFunc, gainFunc, q):
    """
    Creates the classification tree for the given examples. Note that this is implemented - you
    just need to implement makeSubtrees.
