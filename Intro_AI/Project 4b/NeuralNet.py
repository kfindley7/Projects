...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(inActs)):
            ...
            totalModification += abs(new_weights[i] - self.weights[i])
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            inputs = [perceptron.sigmoidActivation(inputs) for perceptron in layer]
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    delta = gPrime * (sum([nextLayer[percepNum].weights[neuronNum + 1] * deltas[0][percepNum] for percepNum in xrange(len(nextLayer))]))
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        ...
        for i, out in enumerate(output[-1]):
            output[-1][i] = int(round(out))
        #print(output, example)
        if output[-1] == example[1]:  #test[1] is a list of output that is testing your trained network results against it
            testCorrect += 1
        else:
            testError += 1
