...
def consistent(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		if constraint.affects(var):
			...
			if not constraint.isSatisfied(assignment.assignedValues[otherVar], value):
				return False
	return True
	...
	for var in domains:
		if not assignment.isAssigned(var):
			if nextVar == None:
				nextVar = var
				tied.append(nextVar)
			elif len(domains[var]) == len(domains[nextVar]):
				if var not in tied:
					tied.append(var)
			elif len(domains[var]) < len(domains[nextVar]):
				nextVar = var
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	for val in values:
		if val not in constraintDict.keys():
			constraintDict[val] = 0
		for constraint in csp.binaryConstraints:
			if constraint.affects(var):
				otherVar = constraint.otherVariable(var)
				if assignment.assignedValues[otherVar] == None and val in assignment.varDomains[otherVar]:
					constraintDict[val] += (len(assignment.varDomains[otherVar]) - 1)
				elif assignment.assignedValues[constraint.otherVariable(var)] == val:
					constraintDict[val] += 1

	lcv = []
	for val in values:
		lcv.append([constraintDict[val],val])
		lcv.sort()
		...
		if consistent(assignment, csp, var, value):
			...
			if inferences != None:	
				...
				if result != None:
					...
					assignment.assignedValues[var] = None
					# reverse inferences in assignment
					for i in inferences:
						assignment.varDomains[i[0]].add(i[1])
			else:
				assignment.assignedValues[var] = None
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		if constraint.affects(var):
			queue[0][1].append(constraint)
	while len(queue) > 0:
		item = queue.pop(0)
		var1, constraints = item[0], item[1]
		for constraint in constraints:
			otherVar = constraint.otherVariable(var1)
			newInferences = revise(assignment, csp, var1, otherVar, constraint)
			if newInferences != None and len(newInferences) != 0:
				...
				temp = [otherVar, []]
				for constraint in csp.binaryConstraints:
					if constraint.affects(otherVar) and constraint.otherVariable(otherVar) != var:
						temp[1].append(constraint)
				queue.append(temp)
			elif newInferences == None:
				for i in inferences:
					assignment.varDomains[i[0]].add(i[1])
				return None
...
def AC3(assignment, csp):
	...
	for var in assignment.varDomains:
		inferences = maintainArcConsistency(assignment, csp, var, None)
		if inferences == None:
			return None
